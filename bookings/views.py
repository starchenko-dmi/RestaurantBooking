...
def reservation_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            available_tables = Table.objects.filter(is_active=True, capacity__gte=guests_count)
...
def reservation_list(request):
    ...
    now = timezone.now()
    active = reservations.filter(date__gte=now.date(), status__in=["pending", "confirmed"])
    archive = reservations.exclude(pk__in=active.values_list("pk", flat=True))

    return render(
        request, "bookings/reservation_list.html", {"active_reservations": active, "archive_reservations": archive}
    )
...
def reservation_cancel(request, pk):
    ...
    return render(request, "bookings/reservation_confirm_cancel.html", {"reservation": reservation})
