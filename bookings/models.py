...
class Table(models.Model):
    ...
    capacity = models.PositiveSmallIntegerField(default=2, verbose_name="Вместимость (человек)")
    zone = models.CharField(max_length=20, choices=ZONE_CHOICES, default="main", verbose_name="Зона")
    ...
    def is_available(self, date, time, duration=2):
        ...
        overlapping_reservations = Reservation.objects.filter(
            ...
            models.Q(time__lt=end_datetime.time(), end_time__gt=start_datetime.time())
        )
...
class Reservation(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="reservations", verbose_name="Пользователь"
    )
    ...
    guests_count = models.PositiveSmallIntegerField(default=2, verbose_name="Количество гостей")
        ...
        if hasattr(self, "table") and self.table:
            ...
            if self.status in ["confirmed", "pending"]:
                ...
                overlapping = (
                    ...
                    .filter(models.Q(time__lt=self.end_time, end_time__gt=self.time))
                )

                if overlapping.exists():
                    raise ValidationError({"time": "Столик уже забронирован на это время"})
    ...
    def is_upcoming(self):
        ...
        now = timezone.now()
        reservation_datetime = timezone.make_aware(timezone.datetime.combine(self.date, self.time))
